...
def get_task_status(date_str, date_format="%Y-%m-%dT%H:%M:%S"):
    ...
    try:
        ...
        if current_datetime >= task_date and current_datetime >= past_boundary:
            return "Past Due"
        elif current_datetime <= task_date and current_datetime <= future_boundary:
            return "In Progress"
        ...
        return "Invalid date format. Please provide dates in the format: YYYY-MM-DDTHH:MM:SS"
